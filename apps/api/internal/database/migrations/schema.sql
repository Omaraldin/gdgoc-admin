...
-- ---------------------------------------------------------------------------
-- issuance_batches: mail template columns (added after initial schema)
-- ---------------------------------------------------------------------------

ALTER TABLE issuance_batches ADD COLUMN IF NOT EXISTS mail_template_id UUID REFERENCES mail_templates(id) ON DELETE SET NULL;
ALTER TABLE issuance_batches ADD COLUMN IF NOT EXISTS mail_variables   JSONB NOT NULL DEFAULT '{}';

-- ---------------------------------------------------------------------------
-- Publish workflow: status column for mail_templates and dynamic_images
-- ---------------------------------------------------------------------------

ALTER TABLE mail_templates ADD COLUMN IF NOT EXISTS
