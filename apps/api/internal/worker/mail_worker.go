...
func NewMailWorker(queue <-chan mail.MailJob, chapterRepo *chapters.Repository, oauthCreds mail.OAuthCreds, maxRetries int) *MailWorker {
	return &MailWorker{queue: queue, chapterRepo: chapterRepo, oauthCreds: oauthCreds, maxRetries: maxRetries}
}
...
func (w *MailWorker) processJob(ctx context.Context, job mail.MailJob) error {
	smtpCfg, err := w.chapterRepo.GetSMTPConfig(ctx, job.ChapterID)
	if err != nil {
		return err
	}
